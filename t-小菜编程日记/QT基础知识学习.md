...
Qt用来通信的机制，**QT独有**，最后通过Moc重新编译生成C++标准代码；辅助生成C++语法规则的语法。
...
5、自定义方式，需要自己声明信号函数和槽函数，QT中有`signals:和 slots:`两种关键字；其中`signals`前面不可加访问权限关键字，`slots`可以添加；信号只需要声明不用定义，编译器会自己生成；其中信号需要单独设置触发的`connect` 使用`emit 关键字来发送`；
